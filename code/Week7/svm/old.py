...
class PlaAndSvm:
    ...
    def is_misclassified(self, point):
        ...
        sign_dot_product = np.sign(np.dot(self.weight_vector, dummy_point))
        if sign_dot_product != point[0]:
            return True
        return False
    ...
    def get_svm_error(self, svm_weight_vector):
        svm_points = []
        svm_point_scores = []
        for point in self.out_points:
            svm_point = [point[1], point[2]]
        ...
        for i in range(0, len(svm_points)):
            dot_sign = np.sign(np.dot(svm_points[i], svm_weight_vector))
            if dot_sign != svm_point_scores[i]:
                count += 1
        print 'begin'
        print count
        print len(svm_points)
    ...
    def run_svm(self):
        ...
        for i in range(0, len(sol)):
            if sol[i] > 10 ** -5:
                mult = sol[i] * self.in_point_scores[i];
                svm_weight_vector[0] += mult * self.in_points[i][1]
                svm_weight_vector[1] += mult * self.in_points[i][2]
        print svm_weight_vector
        error = self.get_svm_error(svm_weight_vector)
        print error
        return error
...
if __name__ == '__main__':
    ...
    for i in range(0, num_runs):
        ...
        e_out_svm = p.get_out_classification_error(False)
        if e_out_svm != -1:
            num_svm_runs += 1
    ...
    avg_e_out_pla = sum_e_out_pla / num_runs
